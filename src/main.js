...
const url1= fetch("https://restcountries.com/v2/all").then(res=>res.json()).then(res=>{ 
  ...
  for( let i=0; i<finalCountry.length; i++){
    let paises2= res.find((item) => {
  
      return item.name==finalCountry[i][0]
    })
    console.log(paises2)

    /*paises2.forEach(element => {
      let country_screen= document.createElement("p");
      country_screen.className="btnGreen"
      country_screen.innerHTML= element.name.
...
// //import {rio_teams, timesRepeated /*noRepeated,justFemale,spliceIntoChunks*/} from './data.js'; //las funciones
// //import data from './data/athletes/athletes.js'; 

// ////////////////////////////////////////////////////////
// /*
// const url= await ("https://restcountries.com/v2/all");
// fetch(url)//llamo a la url de la variable
  // .then(response =>response.json())//xq es un json
...
// const mostrarData = (data) =>{
  // //console.log(data)
  // finalCountry.forEach(element => {
    // console.log(element)
    // data.forEach(element => {
      // if (element.name ==finalCountry[0] ) {
...
// function showCountry(datos){
  // const country= document.createElement("div");
  // country.classList.add("country")
  // country.innerHTML= `
  // <div class="country>
    // <div class="country-img">
      // <img src="${datos.flag}" id="some" alt="">
    // </div>
    // <div class= "country-info" id= "hereCountries">
      // <h6>${datos.name}</h6>
    // </div>
  // </div> `
